...
- 在加载阶段会在堆中生成一个代表这个类的java.lang.Class对象，作为方法区这个类的各种数据的访问入口
- Class类的回收条件：
````
...
##准备
* https://blog.csdn.net/itcodexy/article/details/112793206
- 给类的变量(即静态变量，被 static 关键字修饰的变量，只与类相关，因此被称为类变量，不包括实例变量)分配在 Java 堆中，并赋予默认值（由于还没有产生对象，实例变量不在此操作范围内）
- static+final修饰的String类型或者基本类型常量，会直接赋予原值
- static+final修饰的其他引用类型常量，在初始化阶段赋值
...
- 初始化阶段所有被static关键字标识的代码统一执行一遍，如果执行的是静态变量，那么就会使用用户指定的值覆盖之前在准备阶段设置的初始值；
- 如果执行的是static代码块，那么在初始化阶段，JVM就会执行static代码块中定义的所有操作
- 如果执行的是static代码块，那么在初始化阶段，JVM就会执行static代码块中定义的所有操作只会执行一次，哪怕创建多次对象。
...
## 加载类
* new一个对象，默认使用AppClassLoader类的加载器加载
* 类加载器负责根据一个类的全限定名来读取此类的二进制字节流到JVM内部，并存储在运行时内存区的方法区。
